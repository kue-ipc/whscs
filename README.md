...
6. `ansible-playbook -l {{サーバー名}} reboot.yml`
7. `ansible-playbook user_sync.yml`
8. `ansible-playbook health_check.yml`

update_reboot.ymlとreboot.ymlのみサーバー名を指定する。setup.yml、conf_all.yml、user_sync.yml、health_check.ymlはサーバーを指定せずに実行する。ctl,fs,db,lbの場合は、user_sync.ymlを省略してもよい。

FIXME: EL10ではconf_all.yml実行後にファイアウォールルールが読み込まれていない場合があるので、一度再起動を行うこと。

ファイルサーバーの追加は、レプリカ数と同じ数のサーバーが必要になる。追加と同時にrebalanceも実行されるため、ファイルサーバー全体が少し重くなる。
...
管理サーバーのWebにサイト一覧の情報をアップロードする機能がある。管理サーバーのWebサイトの"/webusers.json"でアクセス可能になっている。ユーザー/サイトの情報を変更した場合は、下記の手順で一覧情報を更新することができる。

1. `ansible-playbook user_list.yml`
2. `ansible-playbook conf_mgt.yml`
...
2. `ansible-playbook create_acme_account.yml -e user={{ユーザー名}}`
3. `ansible-playbook create_acme_tls.yml -e user={{ユーザー名}}`
4. `ansible-playbook update_tls.yml -e user={{ユーザー名}}`
5. `../data/webuser/{{ユーザー名}}.yml`でacmeの値をtrueに設定する。
...
- ctlグループをなくし、コントロールサーバーはlocalhostで管理することにしました。
- tplグループをなくし、必要な場合は`-i x.x.x.x,`で指定してセットアップするようにしました。
